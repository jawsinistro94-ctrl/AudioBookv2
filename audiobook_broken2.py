...
class AudioBook:
    def __init__(self, root):
        ...
        self.root.geometry("900x800")  # Increased height to show custom hotkeys list
    ...
    def create_ui(self):
        ...
        # Hotkey list AFTER Best Sellers (row 3)
        ...
        calib_frame.grid(row=4, column=0, columnspan=3, pady=8)
...
5. Click to record the destination position
    ...
    def execute_clicks(self, hotkey_config):
        ...
        try:
            ...
            if hk_type == 'offensive':
                # Offensive rune: Right-click on rune (HUMANIZED movement)
                x1, y1 = clicks[0]['x'], clicks[0]['y']
                print(f"🎯 Moving to rune at ({x1}, {y1})...")
                # Random duration between 0.08-0.12s for human-like movement
                move_duration = random.uniform(0.08, 0.12)
                pyautogui.moveTo(x1, y1, duration=move_duration)
                time.sleep(random.uniform(0.02, 0.04))  # Slight random pause
                ...
                if auto_target:
                    ...
                    if target_pos:
                        # Target found! Move and SINGLE CLICK (HUMANIZED movement)
                        tx, ty = target_pos
                        print(f"🎯 Moving to target at ({tx}, {ty})...")
                        # Fast but humanized movement (0.06-0.10s)
                    ...
                    # MANUAL MODE: Move to fixed destination position (no click) - HUMANIZED
                    if len(clicks) >= 2:
                    ...
                    # Humanized movement speed
                    move_duration = random.uniform(0.10, 0.15)
                ...
                # Delay between clicks (add small random variation)
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r') as f:
                    ...
                    print(f"[LOAD] Config loaded: {len(self.config.get('hotkeys', []))} hotkeys encontrados")
