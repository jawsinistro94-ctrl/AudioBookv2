...
class RoundedButton(tk.Canvas):
    """Modern rounded button with hover effects and optional icon"""
    def __init__(self, parent, text="", command=None, bg_color='#8F3A1C', hover_color='#B44C1E',
                 fg_color='#F9D8A0', font=('Segoe UI', 10, 'bold'), radius=12, 
                 icon=None, width=120, height=40, **kwargs):
    ...
    def draw_button(self):
        ...
        if self.icon:
            icon_x = 20
            self.create_image(icon_x, self.height//2, image=self.icon, anchor='w')
            text_x = icon_x + 30
        else:
            text_x = self.width // 2
...
class RoundedFrame(tk.Canvas):
    """Modern rounded frame container with optional gradient and shadow"""
    def __init__(self, parent, width=400, height=300, radius=15, bg_color='#29140E',
                 border_color='#7A2E1B', border_width=2, shadow=True, gradient=False,
                 gradient_color1=None, gradient_color2=None, **kwargs):
        super().__init__(parent, width=width, height=height, highlightthickness=0, 
                        bg=kwargs.pop('canvas_bg', parent['bg'] if 'bg' in parent.keys() else '#1B0F0B'), **kwargs)
        ...
        self.gradient_color1 = gradient_color1 or bg_color
        self.gradient_color2 = gradient_color2 or bg_color
    ...
    def draw_frame(self):
        ...
        if self.gradient:
            gradient_img = self.create_gradient(self.width, self.height, 
                                               self.gradient_color1, self.gradient_color2)
            self.gradient_photo = ImageTk.PhotoImage(gradient_img)
            self.create_image(0, 0, image=self.gradient_photo, anchor='nw')
        
        if self.border_width > 0:
            self.create_rounded_rect(0, 0, self.width - (4 if self.shadow else 0), 
                                    self.height - (4 if self.shadow else 0), 
                                    self.radius, fill='', outline=self.border_color, 
                                    width=self.border_width)
        
        self.create_rounded_rect(self.border_width, self.border_width, 
                                self.width - self.border_width - (4 if self.shadow else 0), 
                                self.height - self.border_width - (4 if self.shadow else 0), 
...
class ModernCheckbox(tk.Canvas):
    """Modern rounded checkbox with smooth toggle animation"""
    def __init__(self, parent, variable, command=None, size=24, **kwargs):
        super().__init__(parent, width=size, height=size, highlightthickness=0, 
                        bg=kwargs.pop('bg', '#29140E'), **kwargs)
        ...
        self.variable.trace('w', lambda *args: self.update_state())
    ...
    def draw_checkbox(self):
        ...
        radius = 6
        self.create_rounded_rect(2, 2, self.size-2, self.size-2, radius, 
                                fill=fill_color, outline='')
        
        if self.is_checked:
            self.create_line(6, self.size//2, self.size//3, self.size-8, 
                           fill=check_color, width=3)
            self.create_line(self.size//3, self.size-8, self.size-6, 6, 
                           fill=check_color, width=3)
...
class AudioBook:
    def __init__(self, root):
        ...
        # Modern font selection based on platform
        system = platform.system()
        if system == 'Windows':
            self.modern_font = 'Segoe UI'
            self.modern_font_mono = 'Consolas'
        elif system == 'Darwin':
            self.modern_font = 'SF Pro Display'
            self.modern_font_mono = 'SF Mono'
        else:
            self.modern_font = 'DejaVu Sans'
            self.modern_font_mono = 'DejaVu Sans Mono'
        ...
        try:
            ...
            magma_img = magma_img.resize((900, 650), Image.Resampling.LANCZOS)
    ...
    def create_ui(self):
        ...
        # Configure colors with harmonized palette and modern fonts
        ...
        style.configure('TLabel', background=self.colors['bg_primary'], foreground=self.colors['text_body'], font=(self.modern_font, 10))
        style.configure('Title.TLabel', background=self.colors['bg_primary'], foreground=self.colors['text_header'], font=(self.modern_font, 18, 'bold'))
        style.configure('Accent.TButton', background=self.colors['button_default'], foreground=self.colors['text_body'], font=(self.modern_font, 10, 'bold'))
        ...
        # Create main frame (transparent background to show magma)
        main_frame = tk.Frame(self.canvas, bg='', padx=15, pady=15)
        self.canvas.create_window(0, 0, window=main_frame, anchor='nw', width=900, height=650)
        
        # Modern Title Card with gradient and rounded corners
        title_container = tk.Frame(main_frame, bg='')
        title_container.grid(row=0, column=0, columnspan=3, pady=(0, 15), sticky=(tk.W, tk.E))
        
        title_canvas = RoundedFrame(title_container, width=870, height=70, radius=18, 
                                   bg_color=self.colors['bg_secondary'], 
                                   border_color=self.colors['border_highlight'], 
                                   border_width=3, shadow=True, gradient=True,
                                   gradient_color1=self.colors['bg_secondary'],
                                   gradient_color2=self.colors['bg_inset'])
        title_canvas.pack()
        
        title_label = tk.Label(title_canvas, text="AudioBook - Automation System", 
                              font=(self.modern_font, 18, 'bold'), bg=self.colors['bg_secondary'], 
                              fg=self.colors['text_header'])
        title_canvas.create_window(435, 35, window=title_label)
        
        # Modern Status Card with rounded corners
        status_container = tk.Frame(main_frame, bg='')
        status_container.grid(row=1, column=0, columnspan=3, pady=10)
        
        status_canvas = RoundedFrame(status_container, width=870, height=65, radius=15, 
                                    bg_color=self.colors['bg_inset'], 
                                    border_color=self.colors['border'], 
                                    border_width=2, shadow=True)
        status_canvas.pack()
        
        status_inner = tk.Frame(status_canvas, bg=self.colors['bg_inset'])
        status_canvas.create_window(435, 32, window=status_inner)
        
        self.status_label = tk.Label(status_inner, text="ON", 
                                     foreground=self.colors['status_on'], bg=self.colors['bg_inset'],
                                     font=(self.modern_font, 18, 'bold'))
        self.status_label.pack(side=tk.LEFT, padx=15)
        
        # Modern Toggle Button with rounded corners
        self.toggle_btn = RoundedButton(status_inner, text="", command=self.toggle_active,
                                       bg_color=self.colors['status_on'], 
                                       hover_color=self.colors['focus_glow'],
                                       icon=self.power_icon if self.power_icon else None,
                                       width=50, height=50, radius=25, bg=self.colors['bg_inset'])
        self.toggle_btn.pack(side=tk.LEFT, padx=5)
        
        # Modern Hotkey List Card with rounded corners
        list_container = tk.Frame(main_frame, bg='')
        list_container.grid(row=2, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=10)
        
        # Create rounded frame for hotkey list
        list_canvas = tk.Canvas(list_container, width=870, height=250, highlightthickness=0, bg='')
        list_canvas.pack()
        
        # Draw rounded frame background with shadow
        list_rounded = RoundedFrame(list_canvas, width=870, height=250, radius=15,
                                    bg_color=self.colors['bg_inset'],
                                    border_color=self.colors['border_highlight'],
                                    border_width=3, shadow=True, canvas_bg='')
        list_canvas.create_window(0, 0, window=list_rounded, anchor='nw')
        
        # Title with fire icons
        list_title_frame = tk.Frame(list_rounded, bg=self.colors['bg_inset'])
        list_rounded.create_window(435, 20, window=list_title_frame)
        
        if self.fire_icon:
            tk.Label(list_title_frame, image=self.fire_icon, bg=self.colors['bg_inset']).pack(side=tk.LEFT, padx=5)
        
        tk.Label(list_title_frame, text="Hotkeys Configuradas", 
                bg=self.colors['bg_inset'], fg=self.colors['text_header'],
                font=(self.modern_font, 12, 'bold')).pack(side=tk.LEFT, padx=5)
        
        if self.fire_icon:
            tk.Label(list_title_frame, image=self.fire_icon, bg=self.colors['bg_inset']).pack(side=tk.LEFT, padx=5)
        
        # Treeview container
        list_frame = tk.Frame(list_rounded, bg=self.colors['bg_inset'])
        list_rounded.create_window(435, 135, window=list_frame)
        
        # Treeview for hotkeys with modern styling
        columns = ('Hotkey', 'Clicks', 'Delay (ms)')
        self.tree = ttk.Treeview(list_frame, columns=columns, show='headings', height=8)
        
        # Configure Treeview with modern fonts
        style.configure("Treeview", background=self.colors['bg_secondary'], fieldbackground=self.colors['bg_secondary'],
                       foreground=self.colors['text_body'], font=(self.modern_font_mono, 9))
        style.configure("Treeview.Heading", background=self.colors['border'],
                       foreground=self.colors['text_header'], font=(self.modern_font, 10, 'bold'))
        ...
        self.tree.column('Clicks', width=550)
        ...
        # ========== QUICK CONFIGS - Modern Card ==========
        quick_container = tk.Frame(main_frame, bg='')
        quick_container.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)
        
        # Create rounded frame for quick configs
        quick_canvas = tk.Canvas(quick_container, width=870, height=220, highlightthickness=0, bg='')
        quick_canvas.pack()
        
        quick_rounded = RoundedFrame(quick_canvas, width=870, height=220, radius=15,
                                     bg_color=self.colors['bg_inset'],
                                     border_color=self.colors['focus_glow'],
                                     border_width=3, shadow=True, gradient=True,
                                     gradient_color1=self.colors['bg_inset'],
                                     gradient_color2=self.colors['bg_secondary'], canvas_bg='')
        quick_canvas.create_window(0, 0, window=quick_rounded, anchor='nw')
        
        # Title with trophy icons
        quick_title_frame = tk.Frame(quick_rounded, bg=self.colors['bg_inset'])
        quick_rounded.create_window(435, 20, window=quick_title_frame)
        
        if self.trophy_icon:
            tk.Label(quick_title_frame, image=self.trophy_icon, bg=self.colors['bg_inset']).pack(side=tk.LEFT, padx=5)
        ...
        quick_frame = tk.Frame(quick_rounded, bg=self.colors['bg_inset'])
        quick_rounded.create_window(435, 115, window=quick_frame)
        ...
        # Modern Calibration Button with rounded corners
        calib_container = tk.Frame(main_frame, bg='')
        calib_container.grid(row=4, column=0, columnspan=3, pady=8)
        
        calib_text = "Calibrar Auto-Target" if self.sword_icon else "[T] Calibrar Auto-Target"
        self.calib_btn = RoundedButton(calib_container, text=calib_text, 
                                      command=self.calibrate_auto_target,
                                      bg_color=self.colors['button_default'], 
                                      hover_color=self.colors['button_hover'],
                                      fg_color=self.colors['text_body'],
                                      font=(self.modern_font, 10, 'bold'),
                                      icon=self.sword_icon if self.sword_icon else None,
                                      width=220, height=42, radius=12, bg='')
        self.calib_btn.pack(padx=5)
        
        # Modern Management Buttons with rounded corners
        button_container = tk.Frame(main_frame, bg='')
        button_container.grid(row=5, column=0, columnspan=3, pady=10)
        
        RoundedButton(button_container, text="+ Custom Hotkey", 
                     command=self.add_hotkey_dialog,
                     bg_color=self.colors['selection'], 
                     hover_color=self.colors['focus_glow'],
                     fg_color=self.colors['text_header'],
                     font=(self.modern_font, 10, 'bold'),
                     width=160, height=38, radius=10, bg='').pack(side=tk.LEFT, padx=5)
        
        RoundedButton(button_container, text="Editar", 
                     command=self.edit_hotkey,
                     bg_color=self.colors['button_default'], 
                     hover_color=self.colors['button_hover'],
                     fg_color=self.colors['text_body'],
                     font=(self.modern_font, 9),
                     width=100, height=38, radius=10, bg='').pack(side=tk.LEFT, padx=5)
        
        RoundedButton(button_container, text="Deletar", 
                     command=self.delete_hotkey,
                     bg_color=self.colors['button_destructive'], 
                     hover_color='#8B2C2C',
                     fg_color=self.colors['text_body'],
                     font=(self.modern_font, 9),
                     width=100, height=38, radius=10, bg='').pack(side=tk.LEFT, padx=5)
        
        RoundedButton(button_container, text="Limpar Tudo", 
                     command=self.clear_all,
                     bg_color=self.colors['button_destructive'], 
                     hover_color='#8B2C2C',
                     fg_color=self.colors['text_body'],
                     font=(self.modern_font, 9),
                     width=120, height=38, radius=10, bg='').pack(side=tk.LEFT, padx=5)
...
5. Click to record the destination position
    ...
    def start_hotkey_listener(self):
        def on_press(key):
            ...
            try:
                # Ignore OS command keys (Windows/Cmd/Super) - they cause combo contamination
                if key in (keyboard.Key.cmd, keyboard.Key.cmd_l, keyboard.Key.cmd_r):
                    return  # Ignore Windows/Command key completely
                ...
                if key in (keyboard.Key.ctrl, keyboard.Key.ctrl_l, keyboard.Key.ctrl_r):
                    ...
                    k = key.char.lower() if hasattr(key, 'char') else key.name.lower()
                    # Filter out non-alphanumeric special glyphs (▬, etc.)
                    if len(k) == 1 and not k.isalnum() and k not in ['f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9', 'f10', 'f11', 'f12']:
                        return  # Ignore unknown special characters
        ...
        def on_release(key):
            try:
                # Ignore OS command keys (Windows/Cmd/Super)
                if key in (keyboard.Key.cmd, keyboard.Key.cmd_l, keyboard.Key.cmd_r):
                    return
        ...
        try:
            ...
            with mss.mss() as sct:
                ...
                for c in contours:
                    ...
                    # Exclude far edges (battle list, taskbar) - 75% coverage for better detection range
        ...
        try:
            ...
            if hk_type == 'offensive':
                ...
                if auto_target:
                    ...
                    if self.hsv_config and self.hsv_config.get('calibrated', False):
                        print(f"   ✅ USANDO CALIBRAÇÃO!")
                        print(f"      Lower HSV: {self.hsv_config['lower_hsv']}")
                        print(f"      Upper HSV: {self.hsv_config['upper_hsv']}")
                    else:
                        print(f"   ⚠️ SEM CALIBRAÇÃO - usando valores padrão")
    ...
    def execute_quick_sd(self):
        ...
        try:
            ...
            auto_target = sd_config.get('auto_target', True)
            delay = sd_config.get('delay', 100) / 1000.0
            
            # Right-click on SD rune
            x1, y1 = clicks[0]['x'], clicks[0]['y']
            print(f"⚔️ Auto SD: Moving to rune at ({x1}, {y1})...")
            pyautogui.moveTo(x1, y1, duration=0.1)
            time.sleep(0.05)
            pyautogui.click(x1, y1, button='right')
            print(f"⚔️ Auto SD: Right-clicked rune")
            ...
            if auto_target:
                # Scan for target
                print("🎯 Auto SD: Scanning for target...")
                time.sleep(0.1)
                target_pos = self.detect_red_target()
                
                if target_pos:
                    tx, ty = target_pos
                    print(f"🎯 Auto SD: Target found at ({tx}, {ty})")
                    pyautogui.moveTo(tx, ty, duration=0.1)
                    time.sleep(0.05)
                    pyautogui.click(tx, ty, button='left')
                    print(f"✅ Auto SD: Target clicked!")
                else:
                    print("⚠️ Auto SD: No target detected")
            else:
                print("🎮 Auto SD: Manual mode - click target manually")
    ...
    def execute_quick_explo(self):
        ...
        try:
            ...
            auto_target = explo_config.get('auto_target', True)
            delay = explo_config.get('delay', 100) / 1000.0
            
            # Right-click on EXPLO rune
            x1, y1 = clicks[0]['x'], clicks[0]['y']
            print(f"⚔️ Auto EXPLO: Moving to rune at ({x1}, {y1})...")
            pyautogui.moveTo(x1, y1, duration=0.1)
            time.sleep(0.05)
            pyautogui.click(x1, y1, button='right')
            print(f"⚔️ Auto EXPLO: Right-clicked rune")
            ...
            if auto_target:
                # Scan for target
                print("🎯 Auto EXPLO: Scanning for target...")
                time.sleep(0.1)
                target_pos = self.detect_red_target()
                
                if target_pos:
                    tx, ty = target_pos
                    print(f"🎯 Auto EXPLO: Target found at ({tx}, {ty})")
                    pyautogui.moveTo(tx, ty, duration=0.1)
                    time.sleep(0.05)
                    pyautogui.click(tx, ty, button='left')
                    print(f"✅ Auto EXPLO: Target clicked!")
                else:
                    print("⚠️ Auto EXPLO: No target detected")
            else:
                print("🎮 Auto EXPLO: Manual mode - click target manually")
    ...
    def save_config(self):
        """Save hotkeys, quick configs, and HSV calibration to JSON file"""
        try:
            # CRITICAL: Preserve existing profiles if they exist in memory
            # This prevents overwriting calibration when saving other configs
            if not hasattr(self, 'hsv_config'):
                self.hsv_config = {}
            
            # If hsv_config doesn't have profiles but file does, reload from file
                    ...
                    if os.path.exists(self.config_file):
                        with open(self.config_file, 'r') as f:
                            ...
                            if 'profiles' in existing.get('hsv_config', {}):
                                ...
                                print(f"[SAVE] ✅ Restaurou perfis do arquivo antes de salvar")
                except:
                    pass
            ...
            print(f"[SAVE] ✅ Arquivo salvo: {self.config_file}")
        except Exception as e:
    ...
    def load_config(self):
        """Load hotkeys, quick configs, and HSV calibration from JSON file"""
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r') as f:
                ...
                if isinstance(self.config, list):
                    self.hotkeys = self.config
                    self.config = {'hotkeys': self.hotkeys}
                # Handle new format (dict with hotkeys + hsv_config + quick_configs)
                    ...
                    if loaded_hsv:
                        ...
                        if 'profiles' in loaded_hsv:
                            print(f"[LOAD] ✅ Carregou HSV com PERFIS: {list(loaded_hsv['profiles'].keys())}")
                        else:
                            print(f"[LOAD] ⚠️ Carregou HSV SEM perfis (calibrated={loaded_hsv.get('calibrated', False)})")
